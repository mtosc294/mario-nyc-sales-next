    ...
    <div className="grid gap-10 max-[900px]:grid-cols-1 max-sm:gap-8 lg:grid-cols-[.72fr_1.28fr] lg:gap-[70px]">
      <div>
        ...
        <p className="mt-6 text-sm leading-6 text-neutral-500">
          ...
          {" · "}
          <a href={siteConfig.emailHref} className="break-all underline-offset-2 hover:underline">
            {siteConfig.email}
          </a>
        </p>
      </div>
      <div className="rounded-[28px] border border-[var(--line)] bg-white p-5 shadow-[0_25px_60px_rgba(16,27,45,0.06)] max-sm:rounded-2xl sm:p-7">
        ...
        {result ? (
          <div className="grid min-h-[280px] place-items-center text-center max-sm:min-h-[240px] sm:min-h-[360px]">
            <div>
              ...
              <Button className="mt-7 w-full sm:w-auto" onClick={() => setResult(null)}>
                {t("submitAnother")}
              </Button>
            </div>
          </div>
          ...
          <form onSubmit={submit} className="grid grid-cols-1 gap-3.5 sm:grid-cols-2">
            ...
            <label className="grid gap-2 text-[13px] font-bold">
              {t("phone")}
              <Input name="phone" type="tel" placeholder="Optional" className="h-12 rounded-xl text-base" />
            </label>
            <label className="grid gap-2 text-[13px] font-bold">
              {t("timeline")}
              <select
                name="timeline"
                required
                className="h-12 rounded-xl border border-[var(--line)] bg-white px-4 text-base outline-none focus:border-[var(--navy)]"
              >
            ...
            <label className="grid gap-2 text-[13px] font-bold">
              ...
              <Input name="budget" placeholder="$1M-$1.5M" className="h-12 rounded-xl text-base" />
            </label>
            <label className="grid gap-2 text-[13px] font-bold">
              {t("propertyType")}
              <Input name="propertyType" placeholder="Condo, co-op, townhouse..." className="h-12 rounded-xl text-base" />
            </label>
            <label className="grid gap-2 text-[13px] font-bold sm:col-span-2">
              {t("whatShouldIKnow")}
              <textarea
                name="message"
                className="min-h-[100px] rounded-xl border border-[var(--line)] p-3.5 text-base outline-none focus:border-[var(--navy)]"
